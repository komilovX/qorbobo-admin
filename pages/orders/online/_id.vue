            ...
            <el-card shadow="never card">
              ...
              <div class="text item info">
                <div>
                  ...
                  ><span>{{ formaterDate(order.date) }}</span>
                </div>
                <div><strong>Имя клиента: </strong>{{ order.clientName }}</div>
                <div>
                  <strong>Телефон клиента: </strong>{{ order.clientPhone }}
                </div>
            ...
            <el-card shadow="never">
              ...
              >
                ...
                <el-table-column
                  ...
                  show-overflow-tooltip
                />
                <el-table-column label="Общая сумма" align="center">
              ...
              <div class="delivery p1">
                ...
                >{{ formatCurrency(order.delivery) }} sum
              </div>
              <div class="p1 df" style="justify-content: flex-end">
                <b>Итого:</b>&nbsp; {{ calculateTotal }} sum
              </div>
              <div class="df-sb mt1">
                <el-button
                  type="primary"
  ...
  methods: {
    ...
    deleteItem(row) {
      this.$confirm("Вы действительно хотите удалить этот продукт", "Warning", {
        ...
        this.ordered_products = this.ordered_products.filter(
          f => f.id != row.id
        );
      });
    },
    async addProducts() {
      if (!this.products) {
        try {
          this.loading = true;
          const { data, size } = await this.$store.dispatch(
    ...
    addSelectedProduct() {
      this.selectedProducts.forEach(v => {
        ...
        index != -1
          ? (this.ordered_products[index].amount += v.amount)
          : this.ordered_products.push(v);
      });
    ...
    async sendMessage() {
      try {
        if (this.template) {
          ...
          await this.$store.dispatch("send_message/sendNow", {
    ...
    async acceptOrder() {
      this.$confirm(
        "Вы действительно уверенно подтвердить этот заказ?",
        "Info",
        {
          confirmButtonText: "OK",
          cancelButtonText: "Cancel",
        ...
        try {
          ...
          await this.$store.dispatch(
            ...
            JSON.stringify(this.ordered_products)
          );
    ...
    async addToProcess() {
      this.$confirm(
        "Вы действительно хотите добавить этот Заказ в ожидающий?",
        "Warning",
        {
          confirmButtonText: "OK",
          cancelButtonText: "Cancel",
    ...
    async cancelOrder() {
      this.$confirm(
        "Вы действительно уверенно отменить этот заказ?",
        "Warning",
        {
          confirmButtonText: "OK",
          cancelButtonText: "Cancel",
